...
if [ -n “$(git status --porcelain)” ]; then
    ...
    if git push origin ${GITHUB_BRANCH:-main} 2>&1; then
        echo “[$(date '+%H:%M:%S')] ✅ 成功推送到 GitHub”
    else
        echo “[$(date '+%H:%M:%S')] ❌ 推送失败（检查 Token 或网络）”
        git pull --rebase origin ${GITHUB_BRANCH:-main} 2>/dev/null || true
        git push origin ${GITHUB_BRANCH:-main} 2>&1
    fi
else
...
if [ -n “$GITHUB_REPO_URL” ] && [ -n “$GITHUB_TOKEN” ]; then
    echo “GitHub 数据同步已启用 → $GITHUB_REPO_URL”

    if [[ $GITHUB_REPO_URL == https://github.com/* ]]; then
        AUTH_URL=”https://${GITHUB_TOKEN}@${GITHUB_REPO_URL#https://}”
    else
    ...
    (
        ...
        if [ ! -d “.git” ]; then
            ...
            git clone --depth 1 “$AUTH_URL” . 2>/dev/null || {
                echo “仓库为空，初始化新仓库...”
                git init -b ${GITHUB_BRANCH:-main}
                git remote add origin “$AUTH_URL”
                git commit --allow-empty -m “Initial commit”
                git push -u origin ${GITHUB_BRANCH:-main}
            }
        else
            echo “拉取最新云端数据...”
            git pull --rebase origin ${GITHUB_BRANCH:-main} || echo “拉取失败，使用本地数据”
        fi
    )

    echo “已启动后台自动同步（每 ${GITHUB_SYNC_INTERVAL:-60} 秒检查一次）...”
    (
        while true; do
            sleep ${GITHUB_SYNC_INTERVAL:-60}
            /sync.sh
        done
    ) &
else
    echo “未配置 GITHUB_REPO_URL 或 GITHUB_TOKEN，仅本地保存”
fi
